...
export async function appendDecision(entry) {
  const raw = await storageGet([STORAGE_KEYS.logs, STORAGE_KEYS.settings]);
...
export async function getRecentDecisions(limit = 20) {
  const raw = await storageGet([STORAGE_KEYS.logs]);
  const logs = Array.isArray(raw[STORAGE_KEYS.logs]) ? raw[STORAGE_KEYS.logs] : [];
...
export async function getTotalBlockedCount() {
  const raw = await storageGet([STORAGE_KEYS.logs]);
  const logs = Array.isArray(raw[STORAGE_KEYS.logs]) ? raw[STORAGE_KEYS.logs] : [];

  return logs.filter((entry) => entry?.decision === "BLOCK").length;
}
